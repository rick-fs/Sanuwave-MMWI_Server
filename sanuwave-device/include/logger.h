...
class Logger
{
  public:
    ...
    void redirectCerrToLog()
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (logFile_.is_open())
        {
            std::cerr.rdbuf(logFile_.rdbuf());
        }
    }
    ...
    std::string setLogFileWithNoTimestamp(const std::string &baseDir, const std::string &prefix = "log",std::ios_base::openmode mode = std::ios::trunc)
    {
        std::lock_guard<std::mutex> lock(mutex_);
  ...
  private:
    Logger() : minLevel_(LogLevel::TRACE), consoleOutput_(true), currentLogFile_("")
    {
    }
    ~Logger()
    {
